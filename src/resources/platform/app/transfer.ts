...
export async function transferApp(
  ...
  // with body {owner}, differing only by semantics. Split by recipient shape: an
  // email recipient transfers ownership to a personal ACCOUNT; anything else is a
  // TEAM name. Both produce a byte-identical wire request to the legacy
  // app-transfer.ts (a single PATCH with {owner: recipient}), so this is a
