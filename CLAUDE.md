...
- `@heroku/api-client` (local: `../heroku-fetch`) — HTTP client (`HerokuApiClient`) that handles auth (HEROKU_API_KEY / .netrc), headers, and raw fetch calls
- `@heroku/types` (GitHub: `heroku/heroku-types`) — TypeScript types for the Heroku API and the route definition registry that drives dispatch
...
Tests use vitest and mock both external packages (`@heroku/api-client` and `@heroku/types/3.sdk/routes`) via `vi.mock()`. Tests construct mock `Response` objects inline rather than using a shared fixture.
